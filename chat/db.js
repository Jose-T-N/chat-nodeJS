...
async function createTables(){
    ...
    '(id int NOT NULL AUTO_INCREMENT, message VARCHAR(1000) NOT NULL,userID VARCHAR(20) NOT NULL, userSendID VARCHAR(20) NOT NULL,date DATETIME NOT NULL,'+
...
async function addMessage(user,sdUser,msg,date_time){
    const conn = await connect();
    result = '';
    await conn.query('INSERT INTO messages(userID,message,userSendID,date) VALUES ("'+user+'","'+msg+'","'+sdUser+'","'+date_time+'")').then(
        function(){
            result = 'ok';
        }
    ).catch(
...
module.exports = {createDB,createTables,addUser,addMessage,findUser,allUser,findMessageforUser,allMessage,findMessageforDate,findMessageforDateAndUser,findMessageforDateAndSendUser};